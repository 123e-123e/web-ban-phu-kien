<?php
session_start();
// Sử dụng đường dẫn tuyệt đối để không bao giờ bị lạc đường
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['register'])) {
    ...
    if ($password !== $confirm_password) {
        $msg = "<p style='color: #ff4d4d; margin-bottom: 15px;'>Mật khẩu xác nhận không khớp!</p>";
    } else {
        ...
        try {
            ...
            $stmt->execute();
            $msg = "<p style='color: #00ffcc; margin-bottom: 15px;'>Khởi tạo thành công! <a href='login.php' style='color:#fff'>Đăng nhập ngay</a></p>";
            $stmt->close();
        } catch (mysqli_sql_exception $e) {
            if ($e->getCode() == 1062) {
                $msg = "<p style='color: #ff4d4d; margin-bottom: 15px;'>Tên tài khoản hoặc Email đã tồn tại!</p>";
            } else {
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .auth-card { background: rgba(10, 10, 25, 0.7); backdrop-filter: blur(15px); border: 1px solid rgba(160, 32, 240, 0.4); border-radius: 15px; padding: 40px; width: 100%; max-width: 450px; position: relative; text-align: center; }
        .back-home { position: absolute; top: 15px; left: 20px; color: #dfa8ff; text-decoration: none; font-size: 0.85em; }
        h2 { color: #dfa8ff; text-shadow: 0 0 10px #a020f0; margin-bottom: 25px; letter-spacing: 2px; }
...
<body>
    <div class="auth-card">
        ...
        <form action="" method="POST">
            ...
            <div class="input-group"><label>Xác thực mật khẩu</label><input type="password" name="confirm_password" required></div>
            <button type="submit" name="register" class="btn-neon">Khởi tạo</button>
        </form>
        <div class="switch-link">Đã có tài khoản? <a href="login.php">Đăng nhập</a></div>
    </div>
</body>
</html>